...
st.title('📊 DART Value Agent — V1')
st.caption('DART 공시 + 시장데이터 + 재무비율을 결합해 가치평가·숨은 신호·회계 이상징후·매수/매도 조건을 분석합니다.')

with st.sidebar:
    st.header('⚙️ 분석 설정')
    dart_key = st.text_input('DART API Key', type='password', value=st.secrets.get('DART_API_KEY',''))
    openai_key = st.text_input('OpenAI API Key', type='password', value=st.secrets.get('OPENAI_API_KEY',''))
    model = st.text_input('AI Model', value=st.secrets.get('OPENAI_MODEL','gpt-5.6'))
    st.divider()
    st.markdown('**시장 가정**')
    rf = st.number_input('무위험수익률 (%)', value=3.0, step=0.1)
    erp = st.number_input('주식위험프리미엄 (%)', value=5.0, step=0.1)
    terminal_growth = st.number_input('영구성장률 (%)', value=2.0, step=0.1)
    st.caption('WACC/DCF는 V1에서 시장 가정이 필요한 항목을 명시적으로 입력받습니다.')
...
if run:
    if not dart_key:
        st.error('DART_API_KEY를 입력하거나 Streamlit Secrets에 등록해주세요.')
        st.stop()
    ...
    with st.spinner('재무지표와 가치평가를 계산하는 중...'):
        try:
            metrics = build_metrics(corp, annual, rf=rf, erp=erp, terminal_growth=terminal_growth)
    ...
    tabs = st.tabs(['📌 종합판정','💰 가치평가','📈 재무지표','🚨 숨은 신호/회계 이상징후','🤖 AI Agent'])
    ...
    with tabs[0]:
        ...
        for x in score['checks']:
            st.write(('🟢 ' if x['status']=='positive' else '🟡 ' if x['status']=='watch' else '🔴 ') + x['text'])
    ...
    with tabs[3]:
        ...
        for s in signals:
            st.write(('🟢 ' if s['severity']=='low' else '🟡 ' if s['severity']=='medium' else '🔴 ') + s['text'])
        st.subheader('회계 이상징후')
        for a in anomalies:
            st.write(('🔴 ' if a['severity']=='high' else '🟡 ' if a['severity']=='medium' else 'ℹ️ ') + a['text'])
